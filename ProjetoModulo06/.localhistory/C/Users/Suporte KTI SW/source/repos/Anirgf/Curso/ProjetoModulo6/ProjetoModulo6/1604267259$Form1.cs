...
namespace ProjetoModulo6
{
    public partial class Form1 : Form
    {
        ...
        public void CalcularPotencia(Double valorBase, Double ValorExpoente)
        {
            var result = Math.Pow(valorBase, ValorExpoente);
        ...
        private void btnElevaQuadrado_Click(object sender, EventArgs e)
        {
            if (!txtDisplay.Text.Trim().Equals(String.Empty))
            {
                Double valorBase = Convert.ToDouble(txtDisplay.Text.Trim().Replace(".", ","));
                CalcularPotencia(valorBase, 2);
            }
        }
    }
}
